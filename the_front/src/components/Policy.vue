...
const initClass = () => {
  ...
  datalist.map(item => {
    let obj = {
      ...
      ...(item.id === 5 && {
        city: currentCityName.value,
        selectedDistrict: currentCityDistricts.value[selectedDistrictIndex.value] || '全部'
      })
    }
    obj.updateTime = null
    console.log(obj);
...
const getPolicy = async ({ categories, index, keyWord, timePeriod }) => {
  ...
  if (keyWord) {
    ...
    let uniqueRes = []
    //请求两次-解决-适用区域为市时数据缺失的问题
    if(newData.value[5].selectedDistrict=='全部'){
      params.categories[5].selectedDistrict=currentCityName.value
      const res2 = await policyinfoAPI(params)
      uniqueRes = Array.from(new Map([...res.data.row, ...res2.data.row].map(item => [item.id, item])).values());
      // console.log(uniqueRes)
    }
    if (res.status != 0) return
    total.value = uniqueRes.length!==0?uniqueRes.length:res.data.total 
    policyinfoData.value = uniqueRes.length!==0?uniqueRes:res.data.row
    console.log(params);
  }
