...
import { policyModelAPI, policyModelUnqualified, postEnterpriseDataAPI } from '@/api/service'
...
const submitUn = async () => {
  ...
  const mergedData = [...obj.value, ...unqualDate.value]
...
<template>
      ...
      <div class="policy_left">
        <div class="el_card" v-for="(item, index) in tableDataLeft" :key="index">
          <div class="el_card_top">
            ...
            <div class="second">
              <span @click="toSumbDataFun(item.modelId)">{{ isDaBiao(item.indicatorName) == '未达标' ? "补全资料" : "修改资料" }}</span>
          ...
          <div class="el_card_bottom">
            <span>
              <font class="dabiao" :class="{ activedabiao: isDaBiao(item.indicatorName) == '未达标' }">
                {{ isDaBiao(item.indicatorName) }}</font>
              <font class="suxian">|</font>
              <font class="shenbao" :class="{ activeshenbao: isShenBao(item.history) == '历史已申报' }">
    ...
    <div class="main_center">
      ...
      <div class="modelName_div">
        ...
        <div class="caozuo">
          ...
          <span class="shenbao" @click="toSumbMaterialFun(ZT.split('/')[0])"
            style="margin-left: 0.3vw;background:linear-gradient(45deg, #1FB456,#35C97F) ;">提交材料</span>
        </div>
  ...
  <el-dialog style="width: 45%;" v-model="isVisble" :close-on-click-modal="false">
    <div class="main_center">
  ...
  <el-dialog style="width: 45%;" v-model="isVisble2" :close-on-click-modal="false" :before-close="handleClose">
    <div class="main_center">
          ...
          <div style="height:20vw">
            <el-scrollbar>
              <div style="height: 100%;">
                <el-row :gutter="20" v-for="item in unqualDate" :key="item"> <!-- gutter 是列与列之间的间距 -->
                  <el-col :span="12"> <!-- 占据 50% 的宽度 -->
                    <h5>{{ item.name }}</h5>
                  </el-col>
                  <el-col :span="12"> <!-- 占据 50% 的宽度 -->
                    <el-input style="width: 100%;" type="text" v-model="item.value" />
                  </el-col>
                </el-row>
              </div>
            </el-scrollbar>
