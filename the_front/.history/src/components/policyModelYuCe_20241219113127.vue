...
import {policyModelAPI} from '@/api/service'
...
const users = ref({});
const N = ref(0);
const getPolicyModelData=async (Year)=>{
  const info = getCookie()
    if(info){
        users.value=info
    }
  const res=await policyModelAPI({id:users.value.id,year:Year})
...
const changeYear=(year)=>{
  ...
  Year.value=year
  getPolicyModelData(Year.value)
  // console.log(Year.value)
  // getHuiXiangFun({id:users.value.id,year:Year.value})
}
...
//     DaBiaoStr.value=`该政策模型期望的指标条件:${obj1[0].expectIndicator}`
//     QiYeStr.value=`企业真实指标条件:${obj1[0].yourIndicator}`
//   }
...
<template>
  <div id="main">
    <div class="content_head">
      ...
      <h1>政策预测模型</h1>
      <span v-if="tableDataLeft.length==0" class="nodata">NO DATA</span>
      <div id="content">
        ...
        >
          <div class="el_card_top">
              ...
              <div class="second">
                <span @click="toSumbDataFun">{{isDaBiao(item.indicatorName)=='未达标'?"补全资料":"修改资料"}}</span>
                <!-- <span style="margin-left: 0.3vw;background:linear-gradient(45deg, #1FB456,#35C97F) ;"
                @click="toSumbMaterialFun(isDaBiao(item.indicatorName))"
               >提交材料</span> -->
              </div>
          </div>
          <div class="el_card_bottom">
              <span><font class="dabiao" :class="{activedabiao:isDaBiao(item.indicatorName)=='未达标'}">{{isDaBiao(item.indicatorName)}}</font>
                <font class="suxian">|</font>
                <font class="shenbao" :class="{activeshenbao:isShenBao(item.history)=='历史已申报'}">{{isShenBao(item.history)}}</font></span>
      ...
      <div class="policy_right">
        ...
        >
          ...
          <div class="el_card_bottom">
            <span><font class="dabiao" :class="{activedabiao:isDaBiao(item.indicatorName)=='未达标'}">{{isDaBiao(item.indicatorName)}}</font><font class="suxian">|</font><font class="shenbao">{{isShenBao(item.history)}}</font></span><span style="cursor: pointer;" @click="showMore(item)">查看更多></span>
        </div>
        </div> 
      </div>
    </div>
...
<el-dialog 
  ...
  >
      ...
      <div class="upload">
        <p>您的政策状态为:<span style="margin-left:0.5vw">
        ...
        <p v-if="QiYeStr">
          企业真实指标条件:<span style="color: red;margin-left:0.5vw">{{ QiYeStr }}</span>
        </p>
        <p v-if="LiShiStr">
           <span>{{ LiShiStr }}</span>
        </p>
  ...
  <el-dialog style="width: 45%;" v-model="isVisble"
:close-on-click-modal="false"
>
  <div class="main_center">
